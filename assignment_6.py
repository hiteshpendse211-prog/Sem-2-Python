class library:
 ...
 def check_out(self):
   if self.is_avaiblable:
    self.is_avaible = False
    print(f"'{self.book_name}' has been checked out.") 
   else:
    print(f"Sorry, '{self.book_name}' is already checked out.")
  def return_book(self):
   if not self.is_avaiblable:
    self.is_avaiblable = True
    print(f"'{slef.book_name}' has been returned.")
   else:
    print(f"'{self.book_name}' was not checked out.")
   def display_book(self):
    status = "Available" if slef.is_avaiblable else "Not Available"
...
book2 = Library("C++ Basics", "Bjare Stroustrup")
